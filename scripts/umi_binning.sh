...
if [ -z ${TRIM_FLAG+x} ]; then
  ...
  cat $READ_IN | $GNUPARALLEL --progress -j 10 -L 4 --round-robin --pipe \
    ...
    $PORECHOP_UMI \
      ...
      --middle_threshold 80 \
      --check_reads 5000; \
    $VSEARCH --fastq_filter $TRIM_DIR/{#}_trim.tmp --fastq_ascii 33 --fastq_qmax 93 --fastq_maxee_rate ${MAX_EE} --fastqout $TRIM_DIR/{#}_filt1.tmp ;\
...
$CUTADAPT -j $THREADS -e 0.1 -O 11 -m 18 -M 18 \
  --discard-untrimmed \
  -g $FW1...$FW2 -g $RV1...$RV2 \
