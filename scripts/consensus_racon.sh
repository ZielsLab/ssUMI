...
where:
    ...
    -d  Directory containing bins and binning info. UMI read bins are located in subdirectory 'bins/' in the format
        'umi*bins.fastq'. Recursive search. List of bin files to process must be in pass_bins.txt in directory.
    -o  Output directory.
...
seed_racon () {
 ...
 # Make sure UMI bin is worthy
  if grep -Fxq "$UMINO" $BINDIR/pass_bins.txt; then
  ...
  # Find seed read
  $USEARCH -cluster_fast $RB -id 0.95 -strand both\
  ...
  # Map to consensus to see subread divergence
    #$MINIMAP2 \
    #  -t 1 \
    #  -ax map-ont \
    #  $OUT/${UMINO}_sr.fa \
...
find ${IN}/bins -name 'umi*bins.fastq'  |\
  ( [[ -f "${SAMPLE}" ]] && grep -Ff $SAMPLE || cat ) |\
  $GNUPARALLEL \
    --env seed_racon \
    --progress \
    -j $THREADS \
    "seed_racon {} $OUT $ROUNDS $PRESET $RACON_ARG $IN"
...
#  -name "*.sam" \
#  -exec cat {} \; \
#  > $OUT/consensus_${OUT_NAME}_intrabin.sam
