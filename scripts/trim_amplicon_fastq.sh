...
USAGE="
-- longread_umi trim_amplicon_fastq: Trimming sequences based on primers
...
cutadapt_wrapper(){
  ...
  $CUTADAPT \
    ...
    -M $MAX_LENGTH \
    -g $FW2...$RV2R \
    $IN > $OUT_DIR/${IN_NAME}.fastq 2> $LOG_DIR/${IN_NAME}_amplicon_trim_log.txt
  ...
  $SEQTK seq -r $OUT_DIR/$IN_NAME.tmp |\
    $CUTADAPT \
      ...
      -m $MIN_LENGTH \
      -M $MAX_LENGTH \
      -g $FW2...$RV2R - >> $OUT_DIR/${IN_NAME}.fastq 2>> $LOG_DIR/${IN_NAME}_amplicon_trim_log.txt
